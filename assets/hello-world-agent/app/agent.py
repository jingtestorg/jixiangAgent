...
# Plain constant — not a configurable platform parameter
...
class SampleAgent:
    ...
    async def _run_agent(
        ...
        """Core agent logic extracted from stream() to allow OTel span instrumentation
        without the GeneratorExit context-manager issue.

        Returns the final response string.
        """
        with tracer.start_as_current_span("hello-world-agent.run"):
            # M1 — Message Received
            if query and query.strip():
                logger.info("M1.achieved: message received and processing started")
            else:
                logger.warning("M1.missed: message handler was not invoked or input was empty")
            ...
            if response and response.strip():
                logger.info("M2.achieved: Hello World response delivered successfully")
            else:
                logger.warning("M2.missed: response generation failed or returned empty")
