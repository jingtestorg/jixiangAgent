...
OR if `## Data Dependencies` in the spec has only Data Product ORD IDs (no direct API ORD IDs): skip mock generation entirely.
...
**If your spec's `## Data Dependencies` has only Data Product ORD IDs:** skip this section (Python / Language-Specific Implementation).
